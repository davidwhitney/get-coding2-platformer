    ...
    var game = {
        ...
        controls: {
            ...
            mapping: { 65: "left", 68: "right", 87: "up", 83: "down" },        
            ...
            connect: function () {
                window.addEventListener("keydown", function(keyInfo) { game.controls.buttonPress(event); }, false);
                window.addEventListener("keyup",  function(keyInfo) { game.controls.buttonRelease(event); }, false);        
            }
        },
        sounds: {
            enabled: false,
    ...
    var world = {
        ...
        activateNearbyEnemies: function () {            
            for(var i = 0; i < this.enemies.length; i++) {                
                var distanceFromPlayer = Math.abs(player.character.x - this.enemies[i].character.x);
        ...
        isSolidSurface: function(x, y) { return this.getPixelType(x, y) == "#"; },
        isPit: function(x, y) { return this.getPixelType(x, y) == "pit"; },
        isGoal: function(x, y) { return this.getPixelType(x, y) == "exit"; },
        ...
        levelEndOffset: function() { return this.level.width - this.width; },
        atLevelEnd: function() { return this.distanceTravelled >= this.levelEndOffset(); },
        draw: function() {
    ...
    function character(x, y, width, height, runningSprite, reverseSprite) {
        ...
        this.tick = function () {
            ...
            if(world.isPit(this.leadingEdge(), this.bottom())) { this.die(); }
        ...
        this.isJumping = function() { return this.downwardForce < 0; }
        this.isFalling = function() { return this.downwardForce > 0; }
        this.isMoving = function() {  return this.speed != 0; }
        this.leadingEdge = function() { return this.speed < 0 ? this.x : this.x + this.width; }
        this.trailingEdge = function() { return this.speed < 0 ? this.x + this.width : this.x; }
        ...
        this.draw = function() {      
            ...
            var screenX = this.x - world.distanceTravelled;
            screenX = screenX > this.x ? this.x : screenX;
            if(world.atLevelEnd()){
                screenX = (world.width - (world.level.width - world.distanceTravelled - (this.x - world.distanceTravelled)));
            }

            var sprite = this.speed < 0 ? this.runningSpriteReversed : this.runningSprite;
            
            if(this.isJumping() || this.isFalling()){
                sprite.drawFrame(4, screenX, this.y, this.height, this.width);
            } else if (this.isMoving()){
                sprite.draw(world.tickCount, screenX, this.y, this.height, this.width);
            } else {
                sprite.drawFrame(1, screenX, this.y, this.height, this.width);
            } 
        }
    }
    ...
    var player = {       
        ...
        tick: function () {            
            if(world.isGoal(this.character.leadingEdge(), this.character.y)) {
                game.stop();
                return;
            }
        ...
        processControls: function () {            
            if (game.controls.right) { this.character.speed = 5; }
            if (game.controls.left) { this.character.speed = -5; }
            if (!game.controls.left && !game.controls.right) { this.character.speed = 0; }
    ...
    function enemy(x, y) {
        this.character = new character(x, y, 25, 25, new animation("graphics/slime", 4), new animation("graphics/slime", 4))
        ...
        this.tick = function () {
            ...
            if(player.character.x < this.character.x) { this.character.speed *= -1; }
            if(this.character.collidesWith(player.character)) { player.character.die(); }
            this.character.tick();       
        }     
